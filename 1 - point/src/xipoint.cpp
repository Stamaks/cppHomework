...
namespace xi {
    ...
    /// else resize, move all points after pos to the right, place new point to pos
    ...
    /// if pos >= array size do nothing
    /// if array size == 1 clear the array
    /// else create new array, put all points (without the one in pos) there
    void PointArray::remove(PointArray::Uint pos)
    {
        ...
        {
            auto newArray = new Point[getSize()-1];

            for (PointArray::Uint i = 0; i < pos; ++i)
            {
